...
describe('applyFixes', () => {
  it('opens one PR with every accepted file fix and marks the findings fixed', async () => {
    ...
    assert.deepEqual(
      callsTo(octokit, 'pulls.requestReviewers').map(
  ...
  it('falls back to CODEOWNERS owners, then contributors, and says so in the body', async () => {
    ...
    assert.match(
      callsTo(octokit, 'pulls.create')[0].body,
      /Assigned people from CODEOWNERS/
    )
    assert.deepEqual(callsTo(octokit, 'pulls.requestReviewers')[0].reviewers, [
      'writer',
    ])
    ...
    assert.match(
      callsTo(octokit2, 'pulls.create')[0].body,
      /Assigned repo contributors/
    )
    assert.deepEqual(callsTo(octokit2, 'pulls.requestReviewers')[0].reviewers, [
      'alice',
    ])
  })
  ...
  it('commits a leading-slash fix path without the slash and exposes it the same way', async () => {
    ...
    assert.deepEqual(
      callsTo(snapshot.octokit as never, 'repos.createOrUpdateFileContents')[0]
        .path,
      'x'
    )
  ...
  it('leaves findings the runner already failed alone', async () => {
    ...
    assert.equal(callsTo(snapshot.octokit as never, 'pulls.create').length, 0)
  })
})
