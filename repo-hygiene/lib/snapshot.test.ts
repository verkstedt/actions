...
describe('takeSnapshot', () => {
  it('fetches only the head SHA up front', async () => {
    ...
    assert.deepEqual(listCallsTo(octokit, 'git.getRef'), [
      { owner: 'org', repo: 'r', ref: 'heads/main' },
    ])
    assert.equal(listCallsTo(octokit, 'git.getTree').length, 0)
    assert.equal(listCallsTo(octokit, 'pulls.list').length, 0)
  })
  ...
  it('lists paths once, prefixed with a slash, and warns on truncation', async () => {
    ...
    assert.equal(listCallsTo(octokit, 'git.getTree').length, 1)
    assert.deepEqual(listCallsTo(octokit, 'git.getCommit'), [
  ...
  it('reads files pinned to the head SHA and memoises them', async () => {
    ...
    const reads = listCallsTo(octokit, 'repos.getContent')
    assert.deepEqual(
  ...
  it('returns the first existing candidate', async () => {
    ...
    const file = await snapshot.readFirstExistingFile([
      '.github/CODEOWNERS',
      'CODEOWNERS',
      'docs/CODEOWNERS',
    ])
    ...
    assert.equal(
      await snapshot.readFirstExistingFile([
        '.github/CODEOWNERS',
        'docs/CODEOWNERS',
      ]),
      null
  ...
  it('records which paths a check read, including misses', async () => {
    ...
    await snapshot.readFirstExistingFile(['b', 'c'])
  ...
  it('lists open PRs once', async () => {
    ...
    assert.equal(listCallsTo(octokit, 'pulls.list').length, 1)
  })
