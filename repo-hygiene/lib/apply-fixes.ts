...
import { commitChange, errorMessage } from './github.ts'
import {
  CODEOWNERS_PATHS,
  chooseReviewers,
  describeReviewers,
...
function finalFiles(findings: Array<FileFinding>): Map<string, string> {
  const files = new Map<string, string>()
  for (const { fix } of findings) files.set(normalise(fix.path), fix.content)
  return files
}
...
async function committedCodeowners(snapshot: RepoSnapshot) {
...
async function openPr(
  ...
  snapshot: RepoSnapshot,
  run: RunOptions
): Promise<{ pr: PullRequest; requested: Array<string> }> {
...
function outcomeForActionResult(
  fix: ActionFix,
