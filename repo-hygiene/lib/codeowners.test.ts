...
`)
  const owners = (file: string) => findCodeownersFor(file, lines)
  ...
  it('returns null when nothing matches', () => {
    assert.equal(findCodeownersFor('x', parseCodeowners('/y @a')), null)
  })
...
`)
    assert.deepEqual(findCodeownersFor('file1.js', bracketLines), ['@js-owner'])
    assert.deepEqual(findCodeownersFor('file[12].js', bracketLines), [
      '@js-owner',
    ])
  })
  ...
  it('unions owners of several files in first-seen order', () => {
    assert.deepEqual(
      collectCodeownersForFiles(
        ['scripts/a.sh', 'foo.js', 'apps/github'],
        lines
      ),
      ['@doctocat', '@octocat', '@js-owner']
    )
    assert.deepEqual(collectCodeownersForFiles(['apps/github'], lines), [])
  })
})
