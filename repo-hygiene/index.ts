...
function failedRepo(org: string, repoMeta: RepoMeta, e: unknown): Finding {
  return {
...
async function main(): Promise<void> {
  ...
  if (inputs.dryRun) await core.summary.addRaw(DRY_RUN_NOTE).write()
  ...
  for (const check of checks) await check.setup?.(octokit)
  core.info(
  ...
  for (const [index, repoMeta] of repos.entries()) {
    ...
    try {
      ...
      if (audited.preview) previews.push(audited.preview)
    } catch (e) {
...
main().catch((e: unknown) => core.setFailed(errorMessage(e)))
